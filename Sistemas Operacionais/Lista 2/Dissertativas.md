...
R: Um processo é um programa em execução. Uma thread (ou "linha de execução") é a menor unidade de execução dentro de um processo. Um processo pode ter uma ou várias threads, elas compartilham dados entre si, ja os processos não
...
R: 510 ou 105, os dois processos não se comunicam, possuem 2 copias de 'var' e concorrem um contra o outro
b) Como vocẽ resolveria o problema para imprimir o resultado correto? Por que você escolheu essa solução?
Usando PIPE, é uma das opções mais simples e diretas tambem permite que muitos outros valores (deixando o código mais robusto), não exige memória compartilhada
d) Quais seriam as abordagens possíveis na sua opinião? (cite ao menos as vistas em aula).
PIPE, memória compartilhada, tambêm é possivel usar wait() na função pai e passar o valor de var para a função pai por exit(var) mas somente funciona
...
int main() {
    ...
    if (fork() != 0) {     // PAI
        ...
        // lê o valor enviado pelo filho
        read(fd[0], &child_value, sizeof(child_value));
        ...
        var += 10;         // valor do filho

        // envia valor para o pai
